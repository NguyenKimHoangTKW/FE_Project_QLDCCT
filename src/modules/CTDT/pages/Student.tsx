...
export default function StudentCTDTInterface() {
    ...
    const ShowData = async () => {
        setLoading(true);
        try {
            const res = await GetListStudentCTDTAPI.GetListStudent({ id_program: Number(optionData.id_program), id_class: Number(optionData.id_class), Page: page, PageSize: pageSize });
            if (res.success) {
