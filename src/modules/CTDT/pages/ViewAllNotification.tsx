...
import { NotificationCTDTAPI } from "../../../api/CTDT/Notification";
import { unixTimestampToDate } from "../../../URL_Config";

export default function ViewAllNotificationCTDT() {
    ...
    const ShowNotification = async () => {
        const res = await NotificationCTDTAPI.GetAllNotification({ Page: page, PageSize: pageSize });
        if (res.success) {
    ...
    const emitNotificationReload = () => {
        ...
        localStorage.setItem("notification_reload", now);  // cho tab khác (nếu có)
        window.dispatchEvent(new Event("notification_reload")); // cho cùng tab hiện tại
    };
    const ReadNotificationTrue = async (id: number) => {
        const res = await NotificationCTDTAPI.ReadNotificationTrue({ id_notification: id });
        if (res.success) {
            ShowNotification();
    ...
    const ReadNotificationFalse = async (id: number) => {
        const res = await NotificationCTDTAPI.ReadNotificationFalse({ id_notification: id });
        if (res.success) {
            ShowNotification();
            emitNotificationReload();
        }
    ...
    const DeleteNotification = async (id: number) => {
        const res = await NotificationCTDTAPI.DeleteNotification({ id_notification: id });
        if (res.success) {
            ShowNotification();
            emitNotificationReload();
        }
    ...
    const UpdateAllNotificationFalse = async () => {
        const res = await NotificationCTDTAPI.UpdateAllNotificationFalse();
        if (res.success) {
            ShowNotification();
            emitNotificationReload();
        }
    ...
    const UpdateAllNotificationTrue = async () => {
        const res = await NotificationCTDTAPI.UpdateAllNotificationTrue();
        if (res.success) {
            ShowNotification();
            emitNotificationReload();
        }
    ...
    const DeleteAllNotification = async () => {
        const res = await NotificationCTDTAPI.DeleteAllNotification();
        if (res.success) {
            ShowNotification();
            emitNotificationReload();
        }
