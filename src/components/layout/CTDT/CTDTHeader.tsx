...
function CTDTHeaderLayout() {
  ...
  const loadCount = async () => {
    const res = await NotificationCTDTAPI.GetNotificationCount();
    if (res.success) setNotifyCount(res.count);
  };

  const readNotification = async (id_notification: number) => {
    const res = await NotificationCTDTAPI.ReadNotificationTrue({ id_notification: id_notification });
    if (res.success) {
      loadCount();
      loadList();
    }
  ...
  const loadList = async () => {
    const res = await NotificationCTDTAPI.GetNotificationList();
          ...
          >
            ...
            >
              <div className="p-v-15 p-h-25 border-bottom d-flex justify-content-between align-items-center">
                ...
                <Link to="/ctdt/xem-tat-ca-thong-bao" className="btn-sm btn-default btn">
